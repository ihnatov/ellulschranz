...
class GP_Nested_Forms extends GP_Plugin {
	...
	public function get_submitted_nested_entries( $form, $field_id = false, $display_values = true ) {
		...
		foreach( $form['fields'] as $field ) {
			...
			if ( empty( $entry_ids ) && is_callable( 'gravityview_is_single_entry' ) && gravityview_is_single_entry() ) {
				$parent_entry = GFAPI::get_entry( GravityView_frontend::getInstance()->single_entry );
